...
(function(){
	var mapViewSwitch = function(opt){
		var cur = this;
		var stage, radiusX=0, radiusY=0, coorCenter, width, height, tweenAry = [];
		var testAry = [];
		...
		function drawMapBg(){
			...
			new yu.EdgeGradient({ imageData: copyData, width: opt.canvas.width, borderW: 0, height: opt.canvas.height, GradientW: 12, color: [0,165,255] });
		...
		var addDots = function(dotCoors){
			...
			for(var i=0; i<dotCoors.length; i++){
				...
				var num = new createjs.Text(dotCoors[i].num, "14px Arial normal", "#00c6ff");
				num.regY = 7;
				num.x = dot.x + 15;
				num.y = dot.y;
				...
				for(var j=0; j<opt.width; j++){
					if(data[(i*opt.width+j)*4 + 3]!=0){
						...
						if(isOut){
							...
							data[(i*opt.width+j)*4] = 25;
							data[(i*opt.width+j)*4+1] = 91;
							data[(i*opt.width+j)*4+2] = 163;
						}
					}
