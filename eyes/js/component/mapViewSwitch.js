...
(function() {
	var mapViewSwitch = function(opt) {
		...
		function drawMapBg() {
			...
			bg.y = -cur.mapBox.y;
			// bg.alpha = 0.9;
			bg.shadow = new createjs.Shadow("#1085ff", 0, 0, 60);
		...
		var addDots = function(dotCoors) {
			...
			for (var i = 0; i < dotCoors.length; i++) {
				if(!isNaN(dotCoors[i].num) && dotCoors[i].num!=0){
					...
					var num = new createjs.Text(dotCoors[i].num, "normal 18px DIN Medium", "#00c6ff");
					num.regY = 12;
					num.x = dot.x - 14;
					num.y = dot.y;
					num.alpha = 0;
					...
					var tween1 = TweenMax.to(light1, 2.1, {
						...
						onUpdate: function(light1){
							light1.scaleX = light1.scaleY = Math.sin(light1.tweenTime)*1.2;
						}
					})
