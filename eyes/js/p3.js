...
(function(win, doc) {
	...
	p.bindMapNav = function(){
		...
		$(".mapNav span").eq(1).click(function(){
			var obj = { name: $(this).attr("areaName"), id: $(this).attr("id") };
			_this.map.clickProvince(obj,function(){
				_this.changeData();
			});
		});
	...
	p.dealNav = function(id,name){
		...
		$(".mapNav span").each(function(i){
			if(i>index)
				...
				if(i==index)
					$(this).text(name).data({"areaName": name, "areaId": id});
				$(this).show();
			}
		});
	};
	...
	p.initChart = function() {
		...
		_this.p3Chart5.EventDispatcher.on('chartmouseover',function(vet,item){
